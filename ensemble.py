...
def classify(x, y):
    ...
    svr = LinearSVC()
    clf_svm = GridSearchCV(svr, tuned_parameters)
    kf = cross_validation.KFold(len(y_new), n_folds=10)
    accuracy = []
    for train, test in kf:
        predicted_nb = clf_nb.fit(x[train],y_new[train]).predict(x[test])
        predicted_lr = clf_lr.fit(x[train],y_new[train]).predict(x[test])
        predicted_svm = clf_svm.fit(x[train],y_new[train]).predict(x[test])
