...
def get_predictions(knn_neighbors, y_new):
  ...
  """
  predictions=None #np.zeros((len(knn_neighbors),len(y_new[0])))
  #predicted_prob_3 = [y_new[neighbors[0]]+y_new[neighbors[1]]+y_new[neighbors[2]] for neighbors in knn_neighbors]
  predicted_proba=None #np.zeros((len(knn_neighbors),len(y_new[0])), dtype=np.float)
  for neighbors in knn_neighbors:
    ...
    if(predicted_proba is None):
      ...
      predicted_proba=np.array([predicted_labs/(k*1.0)])
    else:
      predicted_proba=np.concatenate((predicted_proba,[predicted_labs/(k*1.0)]))
    label_ones = predicted_labs >= 1
    predicted_labs[label_ones]=1
    if predictions is None:
      predictions=np.array([predicted_labs])
    else:
      #predictions.append(predicted_labs)
      predictions=np.concatenate((predictions,[predicted_labs]))
...
def main():
    ...
    #try for a sample
    sub_coord_dict = {k: coord_dict[k] for k in coord_dict.keys()[0:3500]}
	...
	(predicted, predict_prob) = get_predictions(ng_indices, y_new[train])
	...
	predicted_labels.append(predicted)
        ...
        #if(cvrun>1):
	#  break
    with open('knn_2_class_scores.json', 'wb') as f:
        json.dump(score_per_class, f)
    pickle.dump(score_per_label, open('knn_2_label_scores.p', 'wb'))
    return
