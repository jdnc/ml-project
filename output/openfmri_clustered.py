...
def main():
    ...
    X = openfmri.get_X('zstat_run1.npy')
    y = openfmri.get_Y('cognitive_concepts/cognitive_concepts.txt','data_key_run1.txt','data/new_terms.json')
    # perform clustering, cross validation and classify
    (score_per_class,score_per_label) = classify(X,y)
    with open('openfmri_class_scores.json', 'wb') as f:
        json.dump(score_per_class, f)
    pickle.dump(score_per_label,open('openfmri_label_scores.p','wb'))
    return
